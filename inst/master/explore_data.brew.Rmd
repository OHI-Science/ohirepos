...
stock_catch <- read_csv("stock_catch_by_rgn.csv")
...
rgn_spp_gl_csv <- read_csv("rgn_spp_gl.csv")
...
## save as csv: 
fis_meancatch_lookup %>% write_csv("fis_meancatch_lookup.csv")

```

There are `r fis_meancatch_lookup %>% nrow()` species in the [fis_meancatch](https://github.com/OHI-Science/<%=key%>/blob/master/global_explore/data/fis_meancatch_lookup.csv) layer for <%=study_area%>.
...
These are the species stocks that we have data for [fis_b_bmsy](https://github.com/OHI-Science/<%=key%>/blob/master/global_explore/data/fis_b_bmsy_lookup.csv):
...
## save as csv:
fis_b_bmsy_lookup %>% write_csv("fis_b_bmsy_lookup.csv")
```

There are only `r fis_b_bmsy_lookup %>% nrow()` species in the [fis_b_bmsy](https://github.com/OHI-Science/<%=key%>/blob/master/global_explore/data/fis_b_bmsy_lookup.csv) layer for <%=study_area%>.
...
Let's look at the list of species that are represented in the MAR model as tonnes of harvest: (the [mar_harvest_tonnes](https://github.com/OHI-Science/<%=key%>/blob/master/eez/layers/mar_harvest_tonnes.csv) layer). 
...
np_harvest_tonnes <- np_harvest_tonnes %>%
...
So there are `r nrow(np_harvest_tonnes)` NP products included.
