---
title: "Explore OHI Global Data & Data Gaps for <%=study_area%>"
output:
  html_document:
    toc: true
    toc_float: true
...
stock_catch <- read_csv("stock_catch_by_rgn_kenya.csv")
...
rgn_spp_gl_csv <- read_csv("rgn_spp_gl_kenya.csv")
...
It can also be nice to see it as a figure. There may be a few places where there are overlaps because goals have the exact same scores.
...
- Many goals (`r nrow(gf125)`) have low amounts of gapfilling — under 12.5% (on the y-axis)
- Many goals (`r nrow(s50)`) have scores over 50 (on the x-axis)
...
- TR has the lowest score that has no gapfilled data.

### Species-based goals (ICO, SPP)

- these scores are higher and are not gapfilled (0). 
...
## save as csv: 
fis_meancatch_lookup %>% write_csv("fis_meancatch_lookup_kenya.csv")

```

There are `r fis_meancatch_lookup %>% nrow()` species in the [fis_meancatch](https://github.com/OHI-Science/sam/blob/master/eez/layers/fis_meancatch.csv) layer for <%=study_area%>.
...
stock_catch %>%
  filter(taxon_scientific_name == "Acanthuridae") %>% ## we looked at Katsuwonus pelamis, Etelis, Acanthuridae
...
## save as csv:
fis_b_bmsy_lookup %>% write_csv("fis_b_bmsy_lookup_kenya.csv")
```

There are only `r fis_b_bmsy_lookup %>% nrow()` species in the [fis_b_bmsy](https://github.com/OHI-Science/sam/blob/master/eez/layers/fis_b_bmsy.csv) layer for <%=study_area%>.
...
So there is not great data available through time for <%=study_area%>. 
...
The TR score is low in <%=study_area%>.
