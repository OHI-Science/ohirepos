...
load_scenario = function(scenario){
  ...
  scenario <<- scenario
  dir_scenario <<- file.path(y$data_branch, scenario)
...
create_scenario_rdata = function(scenario, rdata){
  ...
  if (!file.exists(y$data_branch)){

    # git clone data branch
    system(sprintf(
      'git clone --quiet --branch %s %s %s', y$data_branch, y$gh_url, y$data_branch))

  }

  # check dir_scenario subfolder exists
  dir_scenario = file.path(y$data_branch, scenario) # './data/ohi-global/eez2015'
  if (!dir.exists(dir_scenario)) stop(sprintf('Although app.yml data_branch folder "%s" exists, \nit does not contain subfolder "%s" as specified by the scenario in app.yml. \nRecommend deleting local data_branch folder and rerun Shiny app to clone original, or update app.yml to have correct scenario.', y$data_branch, y$scenarios[1]))
