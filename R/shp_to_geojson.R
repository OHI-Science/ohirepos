...
shp_to_geojson = function(shp, js, geojson=sprintf('%s.geojson', tools::file_path_sans_ext(js))){
  ...
  x = st_read(dsn=shp, layer=lyr) #  proj4string=sp::CRS('+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs')
  ...
  output_filename <- paste0(file_path_sans_ext(geojson), ".geojson")

  # Write the GeoJSON file
  st_write(obj = x, dsn = output_filename, driver = "GeoJSON")
